    ...
    while (!stopCriterion()) { // main cycle of the search
        ...
        if (!expand(curNode, map) && listOfDistancesSize > 1) {
            while (curNode.radius > listOfDistances[listOfDistancesSize - 1]) {
                if (tryToDecreaseRadius(curNode, map.getWidth()) && expand(curNode, map)) {
                    break;
                }
            }
        }
...
void LianSearch::update(const Node curNode, Node newNode, bool& successors, const Map& map) {
    ...
    if (pivotRadius > 0 && (newNode.i != map.goal_i || newNode.j != map.goal_j) && !checkPivotCircle(map, newNode)) {
        return;
    }
...
bool LianSearch::expand(const Node curNode, const Map& map) {
    ...
    if (curNode.parent != nullptr) {
        ...
        std::vector<int> candidates { nodeStraightAhead, nodeStraightAhead };

        bool limit1 = true;
        bool limit2 = true;
        
        while (++candidates[0] != --candidates[1] && (limit1 || limit2)) { // until the whole circle is explored or we exceeded anglelimit somewhere
            if (candidates[0] >= curCircleNodes.size())  {
                candidates[0] = 0;
            }

            if (candidates[1] < 0) {
                candidates[1] = curCircleNodes.size() - 1;
            }
            ...
            for (auto cand : candidates) {
                ...
                if ((angle <= 180 && angle <= angleLimit) || (angle > 180 && 360 - angle <= angleLimit)) {
                    ...
                    if (!map.cellOnGrid(new_pos_i, new_pos_j))  {
                        continue;
                    }

                    if (map.cellIsObstacle(new_pos_i, new_pos_j)) {
                        continue;
                    }
        ...
        for (auto node : curCircleNodes) {
            ...
            if (!map.cellOnGrid(newPos_i, newPos_j))  {
                continue;
            }
            if (map.cellIsObstacle(newPos_i, newPos_j)) {
                continue;
            }
